...
%   removal offers to take the glow around the vessels off every frame as well as off
%   the picture, and doing that moves every landmark below by a knowable and large
%   amount without changing how the result looks.  Per the author (2026-08-07) that
%   combination is the user's to make, so this step runs it and warns once per file -
%   see warnIfCubeWasCleaned at the foot of this file for the numbers.
%
%   DETECTION (per trace)
...
%
% See also: runBolus, runBackgroundRemoval, getProductPath
...
%   three lines per recording by design and this is not a stage; runBolus already uses
%   warning this way, so it is not a new idiom.  Reading a settings field a step wrote
%   is a gate on a live CHOICE, not a back-compatibility fallback - a product either
%   carries that settings group or it does not, and both states are current.
