...
%       *_BFI_s.mat   SETTINGS - field settings.runPulsatility added
%
%   EXAMPLE
%     s.nHarm=5;
...
%empty (per-pixel analysis off), exactly like runVasomotion's s.ppxVsmReturn.
...
for fidx=1:1:numel(fNames)
    ...
    if ~isempty(fNames{fidx})
        ...
        for kSig=1:numel(sigNames)
            ...
            %BFI/std(BFI) are runBFI's columns and are intentionally NOT written here.
            switch sigName
                case 'sData'
        ...
        if ~isempty(s.ppxPulsReturn)
            if ~isfield(results,'sMap')
                error('results.sMap not found; runSegmentation must be run before per-pixel pulsatility.');
            end
