%runGuidedIntensity - Per-segment mean intensity at full temporal resolution.
%
...
%
% Syntax:
%    runGuidedIntensity(s, fNames)
%    runGuidedIntensity(s, fNames, fNamesRaw)
%
% Inputs:
%    s         - parameter struct (uses s.libraryFolder for traceability;
%                optional s.memoryCoef, fraction of free RAM to use, default 0.25).
%    fNames    - cell array of segmented *_I_r.mat (or *_K_r.mat) paths; each must
...
%    fNames = getFileNamesList(rootFolder,'*_I_r.mat');
%    runGuidedIntensity(s,fNames(:));
%
% Dependencies: getPointerRLS (.rls); Bio-Formats bfGetReader/bfGetPlane (.cxd).
% See also: runGuidedContrast, runSegmentation, runIntensity, getPointerRLS
%
...
function runGuidedIntensity(s,fNames,fNamesRaw)
...
for i=1:1:numel(fNames)
    ...
    if contains(f,'_I_r.mat')
        ...
        if strcmp(r,f)
            error('runGuidedIntensity:noRawName', ...
