%getFitDLSI - Per-pixel fit of g2 to Dynamic Light Scattering Imaging models.
%
% Syntax:
%    model = getFitDLSI(g2, lags, iniTau)
%    model = getFitDLSI(g2, lags, iniTau, 'Name', Value, ...)
%
% Description:
...
%    model = getFitDLSI(g2, lags, tauC, 'type', 'MDSN');
...
%------------- BEGIN CODE --------------
function model = getFitDLSI(g2, lags, iniTau, varargin)
...
if ~iscell(cLoss) || ~all(cellfun(@ischar, cLoss(:)))
    error('getFitDLSI:cLoss','cLoss must be a char or a cell array of chars.');
end
