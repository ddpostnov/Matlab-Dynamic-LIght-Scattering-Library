% explore_results - Interactive GUI to explore & export publication figures from LSCI results.
%
% WHAT THIS TOOL DOES
%   A single-window app for browsing the *_r.mat RESULTS produced by this library
%   (getSegmentation -> getPulsatility / getVasomotion / getCTTH / setVesselTypes)
...
%
% HOW TO USE IT
%   1. Run  explore_results  (with the library on the path).
...
%     library README and getCategories for the category definitions.
...
function h = explore_results(varargin)
...
% ---- build the window ------------------------------------------------------
fig = uifigure('Name','explore_results - LSCI results explorer', ...
...
%% ==================== RENDERERS ========================================= %%
    ...
    function renderSpectrogram(ax)
        ...
        if isempty(V) || ~isfield(V,'spectrum') || ~isfield(V.spectrum,'amp')
            title(ax,'No stored 2-D spectrum (run getVasomotion with ''spectrum'' in s.segVsmReturn)'); return;
        end
    ...
    function renderPctSpectra(ax)
        ...
        if isempty(V) || ~isfield(V,'fVectors') || ~isfield(V.fVectors,'VB') || ...
            ...
            title(ax,'No percentile spectra (fVectors.VB.ampMeanPct) - run getVasomotion with ''moments'' in s.segVsmReturn'); return;
        end
        rows = selectRows(R.(domainTable(sig)), sel, domainOf(domainTable(sig)));
        w = weightCol(R.(domainTable(sig))); w=w(rows);
    ...
    function renderAmpPct(ax)
        ...
        if isempty(V) || ~isfield(V,'scalars') || ~isfield(V.scalars,'VB') || ...
            ...
            title(ax,'No amplitude percentiles (scalars.VB.ampPct) - run getVasomotion with ''bands'' in s.segVsmReturn'); return;
        end
        rows = selectRows(R.(domainTable(sig)), sel, domainOf(domainTable(sig)));
        w = weightCol(R.(domainTable(sig))); w=w(rows);
