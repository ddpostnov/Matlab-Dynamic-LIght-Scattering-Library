...
%   that getPulsatility applies to every segment cycle and every pixel cycle.
...
%     markers are computed on the reconstruction fData (reproducing getPulsatility,
...
%     computed on the NaN cycle, so - exactly as in getPulsatility - min/max/mean/
...
%   Generalises getPulsatility's frozen five-harmonic preset to nHarm harmonics.
...
%   getPulsatility runs after its (destructive) fit overwrite, here per waveform.
...
% ---- model-free markers (uniform on W; identical expressions to getPulsatility) ----
