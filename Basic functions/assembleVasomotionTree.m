...
%   getVasomotion's per-segment, per-pixel (ppx) and per-segment-averaging (ppxs)
...
%           block in getVasomotion; that shim is deleted together with the temp block.
...
% Normalise `want` onto the Session-2 fine tokens.  New callers (getVasomotion's
