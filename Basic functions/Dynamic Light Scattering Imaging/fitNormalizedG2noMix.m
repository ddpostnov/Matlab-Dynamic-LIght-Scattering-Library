% fitNormalizedG2noMix - fits g2 to DLSI models not allowing mixed dynamics
%
%
% Authors: DD Postnov
...
function model=fitNormalizedG2noMix(g2,fps,iniTau,pointsMin,type,betaMin,betaMax,mask)

%Model types are
% DCO - dynamc component only, n is an extra parameter
% DNC - dynamc component  and noise, n is an extra parameter
% DSC - dynamics and static components, n is an extra parameter
% MSU - miltiple scattering ordered or unordered, single unordered n=0.5 and n=1, d is an extra parameter
% SMO - single static ordered or unordered, multiple - ordered only, n=1 and n=2, m is an extra parameter
% MSUN - MSU, noise included
% SMON - SMO, noise included
% MSUS - MSU, static scattering included
% SMOS - SMO, static scattering included
...
model.range.idx=zeros(size(g2,1),2);
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0 && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    'Lower',[betaMin(i),-1,pMin(i),tauMin],...
                    'Upper',[betaMax(i),1,pMax(i),tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),0,pMax(i)-(pMax(i)-pMin(i))/10,iniTau(i)]);
                fitType = fittype('1+beta*( (p^2)*(exp(-2*((x/tau)^2)))) + (beta^0.5)*2*p*(1-p)*(exp(-((x/tau)^2)))+c','options',fitOpt);
                ts=double(g2Sub{i});
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0 && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    ...
                    'Upper',[betaMax(i),1,pMax(i),tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),0,pMax(i)-(pMax(i)-pMin(i))/10,iniTau(i)]);
                fitType = fittype('1+beta*( (p^2)*(exp(-2*((x/tau))))) + (beta^0.5)*2*p*(1-p)*(exp(-((x/tau))))+c','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0  && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    ...
                    'Upper',[betaMax(i),1,pMax(i),tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),0,pMax(i)-(pMax(i)-pMin(i))/10,iniTau(i)]);
                fitType = fittype('1+beta*( (p^2)*(exp(-2*((x/tau)^0.5)))) + (beta^0.5)*2*p*(1-p)*(exp(-((x/tau)^0.5)))+c','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
...
elseif strcmp(type,'DNC n=2')
    for ii=0:poolSize:size(g2,1)-poolSize
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0 && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    'Lower',[betaMin(i),-1,tauMin],...
                    'Upper',[betaMax(i),1,tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),0,iniTau(i)]);
                fitType = fittype('1+beta*( (exp(-2*((x/tau)^2)))) +c','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0 && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    ...
                    'Upper',[betaMax(i),1,tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),0,iniTau(i)]);
                fitType = fittype('1+beta*( (exp(-2*((x/tau)))))+c','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
...
elseif strcmp(type,'DNC n=0.5')
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0  && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    ...
                    'Upper',[betaMax(i),1,tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),0,iniTau(i)]);
                fitType = fittype('1+beta*( (exp(-2*((x/tau)^0.5))))+c','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
    ...
    for ii=0:poolSize:size(g2,1)-poolSize
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0 && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    'Lower',[betaMin(i),tauMin],...
                    'Upper',[betaMax(i),tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),iniTau(i)]);
                fitType = fittype('1+beta*( (exp(-2*((x/tau)^2))))','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0 && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    ...
                    'Upper',[betaMax(i),tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),iniTau(i)]);
                fitType = fittype('1+beta*( (exp(-2*((x/tau)))))','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
        ...
        parfor i=ii+1:1:ii+poolSize
            if rangeArtifacts(i)==0  && mask(i)==1
                fitOpt = fitoptions('Method','NonlinearLeastSquares',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
                    ...
                    'Upper',[betaMax(i),tauMax],...
                    'StartPoint',[betaMax(i)-0.9*(betaMax(i)-betaMin(i)),iniTau(i)]);
                fitType = fittype('1+beta*( (exp(-2*((x/tau)^0.5))))','options',fitOpt);
                ts=double(g2Sub{i});
                xq=(startLagIdx:1:(length(ts)+1))-1;
                xq=xq./fps;
                [fitobject,gof] = fit(xq',ts,fitType);
