...
function model=fitDLSI_basic_2024(g2,lags,pointsMin,order,type,iniTau,btMin,btMax,bs,mask,isAdaptive)
    ...
    if strcmp(type,'MDSN')
        parfor i=ii+1:ii+poolSize
            if artifacts(i)==0 && mask(i)==1
                ...
                fitType = fittype(['1+',num2str(bs),'*( beta*((p^2)*(d*d*exp(-2*((x/tau)))+(1-d)*(1-d)*exp(-2*((x/tau)^2)) + 2*d*(1-d)*exp(-((x/tau)))*exp(-((x/tau)^2))))+beta*2*p*(1-p)*(d*exp(-((x/tau)))+(1-d)*exp(-((x/tau)^2))))+c'],'options',fitOpt);
                ...
                fitOpt = fitoptions(fitSet{:},...
                    ...
                    'Upper',[btMax(i),1,1,pMax(i),tauMax(i)],...
                    'StartPoint',[btMax(i)-0.9*(btMax(i)-btMin(i)),0,0.1,pMax(i)-(pMax(i)-pMin(i))/10,iniTau(i)]);
                fitType = fittype(['1+',num2str(bs),'*( beta*((p^2)*(d*d*exp(-2*((x/tau)))+(1-d)*(1-d)*exp(-2*((x/tau)^0.5)) +2*d*(1-d)*exp(-((x/tau)^0.5))*exp(-((x/tau)))))+beta*2*p*(1-p)*((1-d)*exp(-((x/tau)^0.5))+d*exp(-((x/tau)))))+c'],'options',fitOpt);
    ...
    elseif strcmp(type,'DSN')
        parfor i=ii+1:ii+poolSize
            if artifacts(i)==0  && mask(i)==1
                ...
                fitType = fittype(['1+',num2str(bs),'*(beta*(p^2)*(exp(-2*((x/tau)^2)))+beta*2*p*(1-p)*(exp(-((x/tau)^2))))+c'],'options',fitOpt);
                ...
                fitOpt = fitoptions(fitSet{:},...
                    ...
                    'Upper',[btMax(i),1,pMax(i),tauMax(i)],...
                    'StartPoint',[btMax(i)-0.9*(btMax(i)-btMin(i)),0,pMax(i)-(pMax(i)-pMin(i))/10,iniTau(i)]);
                fitType = fittype(['1+',num2str(bs),'*(beta*(p^2)*(exp(-2*((x/tau)^1)))+beta*2*p*(1-p)*(exp(-((x/tau)^1))))+c'],'options',fitOpt);
                ...
                fitOpt = fitoptions(fitSet{:},...
                    ...
                    'Upper',[btMax(i),1,pMax(i),tauMax(i)],...
                    'StartPoint',[btMax(i)-0.9*(btMax(i)-btMin(i)),0,pMax(i)-(pMax(i)-pMin(i))/10,iniTau(i)]);
                fitType = fittype(['1+',num2str(bs),'*(beta*(p^2)*(exp(-2*((x/tau)^0.5)))+beta*2*p*(1-p)*(exp(-((x/tau)^0.5))))+c'],'options',fitOpt);
