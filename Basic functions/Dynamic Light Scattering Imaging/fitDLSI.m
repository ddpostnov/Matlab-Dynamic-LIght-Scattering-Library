...
function model=fitDLSI(g2,lags,pointsMin,order,type,iniTau,btMin,btMax,bs,mask)
...
for i=1:1:size(g2,1)
    for ii=1:1:size(g2,2)
        ts=squeeze(g2(i,ii,:));
        model.range.idx(i,ii,2)=find(ts<=max(ts(end),double(1+bs/50)),1,'first');
...
for i=1:1:size(g2,1)
    if artifacts(i)==0 && mask(i)
        g2Sub{i}=squeeze(g2(i,rangeIdxs(i,1):rangeIdxs(i,2)))';
    end
end
...
for ii=0:poolSize:size(g2,1)-poolSize
    ...
    if ii>updStep
        ...
        if sum(validPixels)>100
            ...
            tmp=min(median(tsBeta),max(tsBeta));
    ...
    if strcmp(type,'SOSUMO')
        parfor i=ii+1:ii+poolSize
            if artifacts(i)==0 && mask(i)==1
                ts=double(g2Sub{i});
                xq=lags(lagStart:1:(length(ts)+lagStart-1));
                wt=1./(1:length(ts));
                wt=[0.5,wt(1:end-1)];
                fitOpt = fitoptions('Method','NonlinearLeastSquares','Algorithm','Trust-Region','Display','off','Weights',wt,...%'Robust','Bisquare',...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
    ...
    elseif strcmp(type,'DCO n=2')
        parfor i=ii+1:ii+poolSize
            if artifacts(i)==0  && mask(i)==1
                ...
                wt=1./(1:length(ts));
                wt=[0.5,wt(1:end-1)];
                fitOpt = fitoptions('Method','NonlinearLeastSquares','Algorithm','Trust-Region','Display','off','Weights',wt,...%'MaxFunEvals',3000,'DiffMinChange',10^(-12),'MaxIter',1500,'TolFun',10^(-12),'TolX',10^(-12),...
