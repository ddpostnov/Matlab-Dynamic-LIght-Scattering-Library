...
P(end+1) = mk('Pulsatility', ...
    {'internalCycle','setRegions','splitRegions','segmentation','BFI','pulsatility'}, ...
    {'registration','vesselTypes'}, ...
    'cardiac branch: per-segment pulsatility from the internal cycle');

P(end+1) = mk('Vasomotion', ...
    {'contrast','setRegions','splitRegions','segmentation','dynamicSegmentation','BFI','vasomotion'}, ...
    {'registration','vesselTypes'}, ...
    'contrast branch: slow-dynamics vasomotion metrics');

P(end+1) = mk('NVC', ...
    {'contrast','externalCycle','setRegions','splitRegions','segmentation','dynamicSegmentation', ...
     'BFI','vasomotion'}, ...
    {'registration','vesselTypes'}, ...
    'stimulus-locked epochs on the contrast branch, plus vasomotion');

P(end+1) = mk('Vasoreactivity', ...
    {'contrast','setRegions','splitRegions','segmentation','dynamicSegmentation','BFI'}, ...
    {'registration','vesselTypes'}, ...
    'contrast branch to BFI, no dynamics metric');

P(end+1) = mk('Pulsatility+Vasomotion', ...
    {'contrast','internalCycle','setRegions','splitRegions','segmentation','dynamicSegmentation', ...
