...
% rootFolder = 'O:\HE_BFI-DATA\Mia\AD'; %root folder for the files lookup
% files      = dir(fullfile(rootFolder,'**','*BN.rls')); %<--- use regexp to define the files of interest
...
%OPTION 2 - SET PATH MANUALY, REQUIRES COMMENTING OUT OPTION 1
 fNames{1}="O:\HE_BFI-DATA\scos2025882.rls";
...
%SAME NAMES
files      = dir(fullfile(rootFolder,'**','Roi*t_K_d.mat')); 
fNames     = fullfile({files.folder}', {files.name}');
...
files      = dir(fullfile(rootFolder,'**','Roi*t_K_d.mat')); 
fNames     = fullfile({files.folder}', {files.name}');
fNamesRaw = fullfile({files.folder}',regexprep({files.name}', {'Roi\d_','_t_K_d\.mat$'}, {'','.rls'}));
s.libraryFolder=libraryFolder;
getGuidedContrast(s,fNames,fNamesRaw)
...
for idxA=4 %animals index list, can include all of the animal indexes, e.g.  [1,2,3,4,5] etc
    for idxR=1:2 %ROIs index list - corresponds to the number of rois used in STEP 3
        files      = dir(fullfile(rootFolder,'**',sprintf('Roi%d*LH%03d*t_e_K_d.mat', idxR, idxA))); %Assuming the naming format as advised, e.g. LSCI_202304158_09WT_TN_MLH004BP and ROI splitting
        fNames     = fullfile({files.folder}', {files.name}');
...
%SET FILE NAMES HERE
files      = dir(fullfile(rootFolder,'**','Roi*t_e_K_d.mat'));
fNames     = fullfile({files.folder}', {files.name}');
...
files      = dir(fullfile(rootFolder,'**','Roi*t_e_BFI_d.mat')); %<---ALWAYS REFER TO "_BFI_d.mat" files, but you may use regexp to define specific "_BFI_d.mat" files of interest
fNames     = fullfile({files.folder}', {files.name}');
setTypes(s,fNames);
...
% %to be done ANIMAL by ANIMAL if multiple animals are compared. It can be
% %convienintly done in a loop as below, but REQUIRES proper file naming.
% for idxA=5  %animals index list, can include all of the animal indexes, e.g.  [1,2,3,4,5] etc
%     for idxR=1:2 %ROIs index list - corresponds to the number of rois used in STEP 3
...
%SET FILE NAMES HERE
files      = dir(fullfile(rootFolder,'**','*t_e_BFI_d.mat')); %<---ALWAYS REFER TO "_BFI_d.mat" files, but you may use regexp to define specific "_BFI_d.mat" files of interest
fNames     = fullfile({files.folder}', {files.name}');
