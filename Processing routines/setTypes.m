...
for fidx=1:1:numel(fNames)
    if ~isempty(fNames{fidx})
        ...
        if (fidx==1 &&  s.useReference ) || ~s.useReference
            ...
            % ----- guess sliders 1-12 (computed once) -------------------------------
            ...
            %   sld1     PI(BFI) gradient (this endpoint - next)
            %   sld2     PI(BFI) magnitude
            %   sld3     BFI / diameter ratio
            %   sld4     std(BFI) / BFI ratio
            %   sld5-12  single-column "magnitude" metrics (built by metricSlider):
            %            the time-of-* metrics are inverted, integralRBFI is not.
            % A slider whose source column is absent is inert and not shown. Sliders
            % 1-2 start ON when PI(BFI) exists; the rest start OFF. Default thumbs:
            % sld1 at 0, sld2 at the PI medians around thrsh, sld3 at the data
            % quartiles, and sld4-12 at the metric's parenchymal (category-1) median.
            ...
            % sliders 1 & 2: PI(BFI) gradient and magnitude
            hasPI = any(strcmp('PI(BFI)',vn));
            if hasPI
            ...
            % sliders 5-12: single-column magnitude sliders (parenchymal-median
            % default). The time-of-* metrics are inverted; integralRBFI is not.
            % An absent column yields an inert slider (.has == false), so it won't show.
            sld5  = metricSlider('timeMinBFI',  true);
            sld6  = metricSlider('timeMaxBFI',  true);
            sld7  = metricSlider('integralRBFI',false);
            sld8  = metricSlider('t0B',         true);
            sld9  = metricSlider('t5B',         true);
            sld10 = metricSlider('tUB',         true);
            sld11 = metricSlider('t90',         true);
            sld12 = metricSlider('tMB',         true);
            ...
            % PI(BFI); the rest each rely on their own column.
            present = [hasPI hasPI has3 has4 ...
                sld5.has sld6.has sld7.has sld8.has sld9.has sld10.has sld11.has sld12.has];
            nVis = nnz(present);
            ...
            % 1-2 start ON (PI(BFI) present), the rest start OFF. Sliders whose source
            % column is missing are not built at all.
            sldNames   = {'PI(BFI): this - next','PI(BFI): magnitude', ...
                'BFI / diameter','std(BFI) / BFI', ...
                '-timeMinBFI','-timeMaxBFI','integralRBFI', ...
                '-t0B','-t5B','-tUB','-t90','-tMB'};
            sldRange   = {sld1.rng,sld2.rng,sld3.rng,sld4.rng,sld5.rng,sld6.rng,sld7.rng,sld8.rng,sld9.rng,sld10.rng,sld11.rng,sld12.rng};
            sldLo      = {sld1.lo, sld2.lo, sld3.lo, sld4.lo, sld5.lo, sld6.lo, sld7.lo, sld8.lo, sld9.lo, sld10.lo, sld11.lo, sld12.lo};
            sldHi      = {sld1.hi, sld2.hi, sld3.hi, sld4.hi, sld5.hi, sld6.hi, sld7.hi, sld8.hi, sld9.hi, sld10.hi, sld11.hi, sld12.hi};
            sldSeg     = {sld1.seg,sld2.seg,sld3.seg,sld4.seg,sld5.seg,sld6.seg,sld7.seg,sld8.seg,sld9.seg,sld10.seg,sld11.seg,sld12.seg};
            sldGuess   = true(1,12);                                          % all feed the guess
            sldEnabled = [hasPI hasPI false false false false false false false false false false]; % on by default?
