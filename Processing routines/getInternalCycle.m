            ...
            for i=1:1:length(cyclesDur)
                ...
                if length(pulsesIdxs)>=s.contrastKernelT
                    ...
                    for ii=1:1:length(pulsesIdxs)
                        data(:,:,:,ii)=movmean(readRLS(s.fName,'FramesToSkip',pulsesListFinal(pulsesIdxs(ii),1)-1,'FramesToRead',cyclesDur(i)+s.framesToAverage-1),[0,s.framesToAverage-1],3,'Endpoints','discard');
                    end
