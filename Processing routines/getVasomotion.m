%getVasomotion  Wavelet analysis of vasomotion amplitude and its dynamics
%
%   getVasomotion(s,fNames) loads every *_BFI_d.mat file in fNames (the
%   blood-flow-index datasets produced by getBFI) and characterises the
...
%   resolution gsData from getGuidedContrast/getGuidedIntensity - analysed on
...
%       *_BFI_s.mat   SETTINGS - field settings.getVasomotions added
%
%   EXAMPLE
...
%     getVasomotion(s, fullfile({files.folder}',{files.name}'));
%
%   DEPENDS ON
%     Basic functions/getVasomotionMetrics (shared wavelet vasomotion core),
...
%wavelet coefficients within a segment (via getSegmentation's per-segment sStat,
    ...
    if ~isempty(fNames{fidx})
        ...
        if ~isempty(s.ppxVsmReturn) || ~isempty(s.ppxSegmentAveraging)
            if ~isfield(results,'sMap')
                error('results.sMap not found; getSegmentation must be run before per-pixel analysis.');
            end
...
%   segment (coherent: getSegmentation's sStat then abs; incoherent: mean of |CWT|),
...
%getSegmentation used to build the original segment traces (results.sData), recovered
%from the settings file (settings.getSegmentation.sStat), so ppxs.coherent stays
%comparable to the segment-first sData instead of depending on a separate parameter.
%'mean' -> complex mean; otherwise (getSegmentation's median default) -> robust
%component-wise (real/imag) complex median.
if doCoh
    if isfield(settings,'getSegmentation') && isfield(settings.getSegmentation,'sStat') ...
            && strcmp(settings.getSegmentation.sStat,'mean')
        ppwtStat=@(x) mean(x,3);
    else
...
for combo=1:2
    ...
    for kk=1:nSeg
        ...
        if combo==1
            %coherent: combine the COMPLEX coefficients (getSegmentation's sStat), then abs
