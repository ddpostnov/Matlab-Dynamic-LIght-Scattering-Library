...
%
%   A settings file is a record of HOW a recording was processed.  The two
...
%   that are transport - stageFcn, cancelFcn - and no more, so a hook-free call
%   still saves a byte-identical settings struct.
%
% Syntax:
%    s = reportSettings(s)
%
...
%
% Outputs:
%    s - the same struct with the two hook fields removed (a no-op when they
%        were never there).
%
% Example:
