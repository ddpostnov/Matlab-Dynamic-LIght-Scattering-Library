%readDV - Read an Olympus .dv recording via the Bio-Formats library.
%
%   Reads an Olympus .dv image sequence using Bio-Formats, with optional frame
%   skipping, a frame count and an ROI.
%
% Syntax:
%    [data,sampling,timeStamps,sizeT,startT] = readDV(fileName)
%    [data,sampling,timeStamps,sizeT,startT] = readDV(fileName, framesToSkip, framesToRead, ROI)
%
% Inputs:
%    fileName     - path to the .dv file.
...
%
% Example:
%    [data,dt,ts,sizeT] = readDV('rec.dv',10,100,[10 100;20 200]);
%
% Dependencies: the Bio-Formats MATLAB library (bfGetReader, bfGetPlane).
% See also: readCXD, readMRAW, getK
%
...
%------------- BEGIN CODE --------------
function [data,sampling,timeStamps,sizeT,startT]=readDV(fileName,varargin)

%check if the fileName ends with .dv, add .dv otherwise
C = strsplit(fileName,'.');
if ~strcmp(C{end},'dv')
